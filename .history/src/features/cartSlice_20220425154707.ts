...
export const cartSlice = createSlice({
  ...
  reducers: {
    ...
    add: (state, action) => {
      if (state.cartItems.items[action.payload].count) {
        state.cartItems.items[action.payload].count++;
        state.cartItems.items[action.payload].total =
          state.cartItems.items[action.payload].count *
          state.cartItems.items[action.payload].price;
      }
    },
    subtract: (state, action) => {
      if (state.cartItems.items[action.payload].count !== 1) {
        state.cartItems.items[action.payload].count--;
        state.cartItems.items[action.payload].total =
          state.cartItems.items[action.payload].count *
          state.cartItems.items[action.payload].price;

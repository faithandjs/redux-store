...
export const cartSlice = createSlice({
  ...
  reducers: {
    newItem: (state, action) => {
      if (
        !state.cartItems.items.some((item) => item.id === action.payload.id) ||
        state.cartItems.items.length === 0
      ) {
        const newCart = {
          ...action.payload,
        };
    ...
    add: (state, action) => {
      if (state.cartItems.items[action.payload.index].count) {
        state.cartItems.items[action.payload.index].count++;
        state.cartItems.items[action.payload.index].total =
          state.cartItems.items[action.payload.index].count *
          state.cartItems.items[action.payload.index].price;
      }
    },
    subtract: (state, action) => {
      if (state.cartItems.items[action.payload.index].count !== 1) {
        state.cartItems.items[action.payload.index].count--;
        state.cartItems.items[action.payload.index].total =
          state.cartItems.items[action.payload.index].count *
          state.cartItems.items[action.payload.index].price;

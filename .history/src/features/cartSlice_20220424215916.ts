...
export const cartSlice = createSlice({
  ...
  reducers: {
    newItem: (state, action) => {
      ...
      if (state.cartItems.items.map((item) => item.id !== action.payload.id)) {
        const newCart = {
          ...action.payload,
          message: msg.ADDED,
        };
        state.cartItems.items.push(newCart);
        console.log(newCart.message, '===added');
      } 
    },
    changeMsg: (state, action) =>{ 
      console.log('change msg', action.payload)
      if (state.cartItems.items.some((item) => item.id === action.payload.id)) {
        action.payload = { ...action.payload, message: msg.ALREADYIN };
        console.log(action.payload.message, '===already in');
      } 
    },

...
export const dataSlice = createSlice({
  ...
  reducers: {
    ...
    add: (state, action) => {
      ...
      state.data.items[action.payload.n].count = (current! > 1) ? current! + 1 : current;*/
      console.log(state.data.items[action.payload.n].count, action.payload);
    },
    subtract: (state, action) => {
      if (state.data.items[action.payload.n].count !== 1) {
        state.data.items[action.payload.n].count!--;
      }
      /*let current = state.data.items[action.payload.n].count;
      state.data.items[action.payload.n].count = (current === 1) ? current : current! - 1;*/
      console.log(state.data.items[action.payload.n].count, action.payload);
    },
  },

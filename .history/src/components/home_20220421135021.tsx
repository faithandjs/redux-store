...
function Home() {
  ...
  const [currentRegion, setCurrentRegion] = useState<objectType[]>(data.data.items);
  ...
  switch (data.data.status) {
    ...
    case 3:
      if (current.length !== 0 &&  passed2.current) {
        ...
        pageContent = (
          <div>
            there are no {inputvalue} items in 
  ...
  const categorySelected = () => {
    ...
    if (value === "all") {
      setCurrentRegion(data.data.items);

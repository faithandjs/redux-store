...
export const Message: React.FC<msgProp> = ({ itemE }) => {
  ...
  useEffect(() => {
    console.log(itemE.message);
    cartData.cartItems.items.map((item, index) => {
      if (item.id === itemE.id) {
        const cartItemMsg = cartData.cartItems.items[index].message;
        if (cartItemMsg === msg.ADDED) {
          text.current = <div className="green">{msg.ADDED}</div>;
        }
        if (cartItemMsg === msg.ALREADYIN) {
          text.current = <div className="grey">{msg.ALREADYIN}</div>;
        }
      }
    });

    console.log(text.current.props, msg.ADDED, msg.ALREADYIN);
  }, [cartData]);

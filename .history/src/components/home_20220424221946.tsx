...
import { dataState, objectType, objArr } from "../type";
...
import { bgRotation, CartImg, Message } from "./extras";
        ...
        <div className="details-box">
          ...
          <div className="button-box">
            <button
              onClick={() => {
                if (
                  cartState.cartItems.items.map((itemX) => itemX.id !== item.id)
                ) {
                  console.log('not on 2',  cartState.cartItems.items.some((itemX) => itemX.id !== item.id), item.id)
                  dispatch(newItem(item));
                }
                dispatch(calculateTotal());
                dispatch(changeMsg(item));
              }}
            >
              add to cart
            </button>

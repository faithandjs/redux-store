...
function Cart() {
  ...
  const handleSelection = (item: objectType, s: string) => {
    ...
    if (selected.some((select) => item.id === select.id) && !check.checked) {
      console.log('splice?' , selected.some((select) => item.id === select.id),  !check.checked)
      selected.map((selectItem: objectType, index) => {
        if (selectItem.id === item.id) {
          selected.splice(index, 1);
          console.log('splice', selected)
        }
      });
    } else  if (!selected.some((select) => item.id === select.id) && check.checked) {
      console.log('set? ' , !selected.some((select) => item.id === select.id),  check.checked)
  ...
  console.log("SELECTED: ", selected, "CART", cartData.cartItems.items);
  return (
    <div className="cart">
      <header>
        <Back />
